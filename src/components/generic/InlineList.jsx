...
function InlineListLinkBody({ item, shortenedMode }) {
  ...
  return (
    <div
      className={`inline-link-list-content ${
        item.href ? "" : "text-secondary-2"
      }`}
    >
      {item.img && <ImageView src={item.img} alt={label} className={`me-2`} />}

      {!item.img && (
        <FaIcon
          iconName={item.faIcon}
          colors={item.faIconColors}
          className={`me-1 me-md-2 ${item.href ? "" : "text-secondary-2"}`}
        />
      )}
      <span>{label}</span>
    </div>
